...
- **逐层淌水·前伸导流嘴**:溢流标管移到储水盒前壁,经一段**封闭导流臂**前伸到**下层盆口正上方**(`spout_extra` 调伸入量),嘴端向下滴孔把溢流水滴进下层花盆(底层塞此孔);直叠即对齐,不需错位。

## 当前文件
- **`filesv2/wall_planter_v5.scad`** — 当前基础(单元 W164×H232.5, 真实进深≈199mm, 三轴 ≤240,`Volumes=2`;含 `show_pot` 自检与尺寸 echo)。
