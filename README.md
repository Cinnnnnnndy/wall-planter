...
> **穿插最大化 / 保证重心**:锥盆几乎整段包进箱体,只露盆口;箱深 `box_d` 由几何**自动算出**前伸到盆口后缘(当前 ≈107mm),使满载质心落在底面内、减小前伸悬挑(用 `front_reveal` 调露管量)。
> 形态演进:v4 是"薄背板+浅托盘+悬伸杯"(剖面像平板+花盆);v5 改为长方体让斜锥**穿透**,盆腔为完整锥座,并把盆体深嵌以稳重心。
...
## 当前文件
- **`filesv2/wall_planter_v5.scad`** — 当前基础(单元 164×208×219mm,三轴 ≤240,`Volumes=2`;含 `show_pot` 自检与尺寸 echo)。
