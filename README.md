...
3. **功能分区** — 箱体**下部**=储水/逐层下渗(排水点下方封闭储水盒+溢流标管,底层塞橡胶塞);箱体**上部**=与上层堆叠连接(顶销+半圆缺口);**背面**=平背板开透气孔,并按 `pot_back` 让盆底穿出一截(见下)。
...
箱深 `box_d` 与后伸 `pot_back` 为**可调参数**(盆位 `p0y` 由 `pot_back` 反推);`p0z` 派生(由储水/倾角算)。把 `show_pot=true` 可叠加半透明真花盆做插入自检。
